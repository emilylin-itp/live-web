...
function init() {
  ...
  let fakeX = 0;
  let fakeY = 0;
  let binSize_X = window.innerWidth / 8;
  let binSize_Y = window.innerHeight / 6;
  let originFakeX = window.innerWidth / 16;
  let originFakeY = cwindow.innerHeight / 12;

  function moveFakePick() {

    setInterval(function () {
      fakeX = fakeX + binSize_X;
      fakeY;

      //if fake x is at edge of canvas move fake y down one row
      // then move fake y across the opposite way
      if (fakeX >= canvas.width) {
        fakeY = fakeY + binSize_Y;
        binSize_X = originFakeX;
      }
      ...
      console.log ('binSize X: ' + binSize_X);
      console.log('fakeX: '+ fakeX);

      //function to change which pixel is picked
      //drawRect(fakeX, fakeY);
      fakePick(fakeX, fakeY);
       //moved drawRect function to loop section. doesn't work well here
      // context.fillRect(fakeX, fakeY, 10, 10);
  ...
  function Pixelate(src, dst, opt) {
    ...
    //THIS IS WHERE YOU CHANGE BIN SIZE
    var xBinSize = opt.pixelWidth * 10, //bin size is 80 px
      yBinSize = opt.pixelHeight * 10; //bin size is 80 px
