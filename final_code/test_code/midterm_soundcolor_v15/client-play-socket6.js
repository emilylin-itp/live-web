...
function init() {
    ...
    orangeSelect.addEventListener('click', () => {
        console.log('orange div clicked');
        if (evl != null) {
            canvas.removeEventListener('click', evl);
        }
        evl = drawOrangeSine;
    ...
    function drawRedSine(data) {
        ...
        animateRedSine(data);
        ctx.restore();//pop
        console.log('draw red sine');
    }
    requestAnimationFrame(drawRedSine);
...
function animateRedSine(data) {
    // console.log('drawRedSine function is called!')
    ctx.save();
    plotSine(data.x, data.y, wavelength, step, 0, amplitudeSine, rfreq, "#FD4F4F", lineWidth);
    ctx.restore();
    step += 4; //changes the speed
    window.requestAnimationFrame(animateRedSine);
}

function animateOrangeSine(data) {
    ctx.save();
